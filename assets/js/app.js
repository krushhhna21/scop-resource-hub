...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showLibrary(type) {
    ...
    if (current) {
      ...
      // Year selection for resource-based sections
      function loadYears(type) {
        ...
        for (let i = 1; i <= 6; i++) {
          ...
          btn.addEventListener('click', () => {
            location.hash = '#' + new URLSearchParams({ type, year: i }).toString();
          });
          ...
          console.error('Error loading general resources:', e);
          grid.innerHTML = `<p class="error">Failed to load ${type}. ${e?.message || ''}</p>`;
        }
    const grid = content.querySelector('.grid');
    if (!grid) return;
